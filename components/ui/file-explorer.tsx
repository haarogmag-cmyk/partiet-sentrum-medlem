...
import { Card, CardContent } from '@/components/ui/card' // <--- LA TIL CardContent HER
import { Folder, FileText, ChevronLeft, Plus, Download } from 'lucide-react'
...
export default function FileExplorer({ items, canEdit, onUpload, onCreateFolder, onDownload }: Props) {
  const [currentPath, setCurrentPath] = useState<FileItem[]>([]) 
  const currentFolder = currentPath[currentPath.length - 1]
  const currentFolderId = currentFolder ? currentFolder.id : null

  // Filtrer items som skal vises NÅ
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {visibleItems.map(item => (
                <Card 
                    ...
                    `}
                >
                    <CardContent className="p-4 flex flex-col items-center text-center h-32 justify-center gap-3">
                        ...
                        <div className="w-full">
                            <p className="font-bold text-sm truncate w-full" title={item.title}>{item.title}</p>
                            {!item.is_folder && (
