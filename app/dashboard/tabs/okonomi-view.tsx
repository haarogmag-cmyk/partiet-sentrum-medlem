...
export default async function OkonomiView({ filters, searchParams, user, isSuperAdmin, userRole }: Props) {
  ...
  // 2. FINN KONTEKST (HVILKEN ORG SER VI PÅ?)
  ...
  // SPESIAL-LOGIKK FOR NASJONALT NIVÅ
  if (currentLevel === 'national') {
      const { data: autoMem } = await supabase
        .from('automatic_income_membership')
        .select('*')
        .eq('year', year)
        .eq('org_type', orgType);

      const { data: autoEvt } = await supabase
        .from('automatic_income_events')
        .select('*')
        .eq('year', year)
        // Event view har kanskje ikke org_type direkte, men for nå henter vi alt
        // eller oppdaterer viewet. (Antar viewet er oppdatert)
  ...
  // 5. HELSE-STATISTIKK
  let healthStats: any[] = [];
  let childrenOrgs: any[] = []; // <--- NY LISTE FOR NAVIGASJON

  if (currentLevel !== 'local') {
      // A. Hent data for Helse-sjekken
      ...
      // B. Hent barn for navigasjon
      let childrenQuery = supabase.from('organizations')
      ...
      if (currentLevel === 'national') {
          ...
          childrenQuery = childrenQuery.eq('level', 'county');
      } else if (currentLevel === 'county') {
          const { data: childOrgsData } = await supabase.from('organizations').select('id').eq('parent_id', currentOrgId); 
          const childIds = childOrgsData?.map(o => o.id) || [];
          
          if (childIds.length > 0) {
              healthQuery = healthQuery.in('org_id', childIds);
              childrenQuery = childrenQuery.in('id', childIds);
          } else {
               healthQuery = healthQuery.eq('org_id', '00000000-0000-0000-0000-000000000000'); 
               childrenQuery = childrenQuery.eq('id', '00000000-0000-0000-0000-000000000000');
          }
      }
      ...
      childrenOrgs = childrenData || []; // <--- Fyller listen
  }
  ...
  return (
      <OkonomiTabsClient 
          ...
          userRole={userRole}
          childrenOrgs={childrenOrgs} // <--- Sender listen ned til klienten
