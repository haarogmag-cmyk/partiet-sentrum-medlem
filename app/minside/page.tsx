...
// BYTTET IMPORT HER:
import MemberResourceBrowser from './member-resource-browser'; 
...
export default async function MinSidePage() {
  ...
  // 5. HENT RESSURSER (Både filer og mapper)
      ...
      <div className="w-full max-w-6xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between items-start gap-8">
            <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-6 text-sm text-slate-600">
                    <div><p className="text-[10px] font-bold uppercase text-slate-400 tracking-wider mb-1">E-POST</p><p>{member.email}</p></div>
                    <div><p className="text-[10px] font-bold uppercase text-slate-400 tracking-wider mb-1">MOBIL</p><p>{member.phone}</p></div>
                    <div><p className="text-[10px] font-bold uppercase text-slate-400 tracking-wider mb-1">ADRESSE</p><p>{member.postal_code} {member.city}</p></div>
                    <div><p className="text-[10px] font-bold uppercase text-slate-400 tracking-wider mb-1">LOKALLAG</p><p>{psLokal?.name?.replace('Partiet Sentrum ', '') || 'Ikke funnet'}</p></div>
                </div>
                <div className="mt-6"><EditProfileModal member={member} /></div>
            </div>
            
            <div className="flex flex-col gap-3 items-end">
                <div className="flex gap-3">
                    {isAdmin && <Link href="/dashboard"><Button variant="secondary" className="rounded-full px-6 bg-slate-100 hover:bg-slate-200 border-0">Admin Dashboard</Button></Link>}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="space-y-8">
                <div className="space-y-4">
                    ...
                    <MembershipCard 
                        ...
                        variant="ps"
                        downloadBtn={<DownloadCertificateButton member={member} orgName="Partiet Sentrum" className="bg-white text-[#c93960] border-2 border-white hover:bg-[#c93960] hover:text-white transition-colors font-bold" />}
                    />
                    {isYouth && (
                        <MembershipCard 
                            ...
                            variant="us"
                            downloadBtn={<DownloadCertificateButton member={member} orgName="Unge Sentrum" className="bg-white text-[#8a63d2] border-2 border-white hover:bg-[#8a63d2] hover:text-white transition-colors font-bold" />}
                        />
                    )}
                </div>
            </div>

            {/* MIDTEN (ARRANGEMENTER & RESSURSER) */}
            <div className="lg:col-span-2 space-y-8">
                            ...
                            {nationalEvents.length > 0 ? nationalEvents.map((ev:any) => (
                                <Link key={ev.id} href={`/minside/event/${ev.id}`} className="block hover:bg-slate-50 transition-colors p-6 group">
                                    <div className="flex justify-between items-center">
                                        ...
                                        <div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-400 group-hover:bg-ps-primary group-hover:text-white transition-all">→</div>
                                    </div>
                                </Link>
                            )) : (
                ...
                {/* FRIVILLIG + RESSURSBANK (Nå med FileExplorer!) */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="bg-slate-900 text-white p-8 rounded-3xl shadow-lg relative overflow-hidden flex flex-col">
                         ...
                         <div className="relative z-10 flex-1">
                            ...
                            <p className="text-xs text-slate-400 mb-6">Logoer, maler og dokumenter.</p>
                            
                            {/* HER ER DEN NYE KOMPONENTEN */}
                            <MemberResourceBrowser resources={resources || []} />
...
// ... (Komponentene MembershipCard, EventCard, InfoRow som før) ...
function MembershipCard({ orgName, name, id, status, variant, downloadBtn }: any) {
    const isPaid = status === 'active';
    const bgClass = variant === 'us' ? 'bg-gradient-to-br from-[#8a63d2] to-[#5e1639]' : 'bg-gradient-to-br from-[#c93960] to-[#8a1c3d]';
    return (
        <div className={`relative w-full rounded-3xl shadow-xl overflow-hidden text-white p-8 flex flex-col justify-between h-56 ${bgClass} transition-transform hover:scale-[1.02] duration-300`}>
            ...
            <div className="relative z-10">
                ...
                <p className="font-mono text-xs opacity-60 mb-6 tracking-widest">#{id.slice(0,8).toUpperCase()}</p>
                <div className="flex justify-between items-center">
                    ...
                    <div className="scale-90 origin-right opacity-100">{downloadBtn}</div>
                </div>
            </div>
        </div>
    )
...
function EventCard({ ev, variant = "standard", label }: { ev: any, variant?: "hero" | "standard", label?: string }) {
    return (
        <Link href={`/minside/event/${ev.id}`} className="block h-full">
            <div className={`relative overflow-hidden rounded-3xl transition-all duration-300 group h-full flex flex-col justify-between ${variant === 'hero' ? 'bg-[#c93960] text-white p-8 shadow-lg hover:shadow-xl' : 'bg-white border border-slate-100 p-6 shadow-sm hover:shadow-md hover:border-ps-primary/20'}`}>
                ...
                <div className="relative z-10">
                    ...
                    <h4 className={`font-bold text-xl leading-tight mb-2 ${variant === 'hero' ? 'text-white' : 'text-slate-800 group-hover:text-ps-primary transition-colors'}`}>{ev.title}</h4>
                </div>
